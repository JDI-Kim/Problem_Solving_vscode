/*
    [interview cake] 5. trees and graphs, DFS, iterative way
...
bool isBalanced(const BinaryTreeNode* treeRoot)
{
    ...
    while(!nodes.empty()){
        ...
        if(node->left_ == nullptr && node->right_ == nullptr){
            if( isFirstDepth )
            {
                maxDepth = minDepth = curDepth;
                isFirstDepth = false;
            }
            else
            {
                if( abs(maxDepth - curDepth) > 1 ||
                         abs(minDepth - curDepth) > 1 ) return false;
                if(maxDepth < curDepth) maxDepth = curDepth;
                if(curDepth < minDepth ) minDepth = curDepth;
            }
        }
       
       if(node->left_) nodes.push(make_pair(node->left_, curDepth + 1) );
       if(node->right_)nodes.push(make_pair(node->right_, curDepth + 1) );
